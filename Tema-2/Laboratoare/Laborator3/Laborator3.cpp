...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	modelMatrix *= Transform2D::Translate(1000, 800);
	modelMatrix *= Transform2D::Translate(squareSizee / 2, squareSizee / 2);
	modelMatrix *= Transform2D::Rotate(angularStep);
	modelMatrix *= Transform2D::Translate(-squareSizee / 2, -squareSizee / 2);
	modelMatrix *= Transform2D::Translate(2 * squareSizee, 2 * squareSizee);
	// translateX += deltaTimeSeconds * 100 / restart;
	// translateY += deltaTimeSeconds * 100 / restart;
	// scaleX += 0.01f;
	// scaleY += 0.01f;
	angularStep += M_PI / 250;
	if (angularStep > 2 * M_PI) {
		angularStep = 0;
	}
	if (scaleX > 2.5f) {
		scaleX *= -1;
	...
	if (translateX > window->GetResolution().y - squareSizee) {
		translateX = 0;
		translateY = 0;
		restart = deltaTimeSeconds * 100;
	}
	...
	modelMatrix *= Transform2D::Rotate(angularStep);
	modelMatrix *= Transform2D::Translate(-squareSizee / 2, -squareSizee / 2);
	modelMatrix *= Transform2D::Translate(3 / 2 * squareSizee, 3 / 2 * squareSizee);
