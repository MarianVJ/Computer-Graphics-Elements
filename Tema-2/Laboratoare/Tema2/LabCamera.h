#pragma once
#include <include/glm.h>
#include <include/math.h>
#include <stdio.h>
#include <iostream>

namespace Laborator
{
	class MyCamera
	{
	public:
		MyCamera()
		{
			position = glm::vec3(0, 2, 5);
			forward = glm::vec3(0, 0, -1);
			up = glm::vec3(0, 1, 0);
			right = glm::vec3(1, 0, 0);
			distanceToTarget = 2;
		}

		MyCamera(const glm::vec3 &position, const glm::vec3 &center, const glm::vec3 &up)
		{
			Set(position, center, up);
		}

		~MyCamera()
		{ }

		void Set(const glm::vec3 &position, const glm::vec3 &center, const glm::vec3 &up)
		{
			this->position = position;
			forward = glm::normalize(center - position);
			right = glm::cross(forward, up);
			this->up = glm::cross(right, forward);
		}

		// Update camera
		void Set2(const glm::vec3 &position, const glm::vec3 &center, const glm::vec3 &up,const glm::vec3 &ballPos)
		{
			this->position = position;
			forward = glm::normalize(center - position);
			right = glm::cross(forward, up);
			this->up = glm::cross(right, forward);
			
			this->distanceToTarget = sqrt(pow((ballPos.x - position.x), 2) + pow((ballPos.y - position.y), 2) + pow((ballPos.z - position.z), 2) );
		}

		void MoveForward(float distance)
		{
			glm::vec3 dir = glm::normalize(glm::vec3(forward.x, 0, forward.z));
			// movement will keep the camera at the same height always
			// Example: If you rotate up/down your head and walk forward you will still keep the same relative distance (height) to the ground!
			// Translate the camera using the DIR vector computed from forward
		}

		void TranslateForward(float distance)
		{
			position = position + glm::normalize(forward) * distance;
			// TODO : Translate the camera using the "forward" vector
		}

		void TranslateUpword(float distance)
		{
			position = position + glm::normalize(up) * distance;
			// TODO : Translate the camera using the up vector
		}

		void TranslateRight(float distance)
		{
			// TODO
			// Translate the camera using the "right" vector
			// Usually translation using camera "right' is not very useful because if the camera is rotated around the "forward" vector 
			// translation over the right direction will have an undesired effect; the camera will get closer or farther from the ground
			// Using the projected right vector (onto the ground plane) makes more sense because we will keep the same distance from the ground plane
			position = position + glm::normalize(right) * distance;

		}

		void RotateFirstPerson_OX(float angle)
		{
			// TODO
			// Compute the new "forward" and "up" vectors
			// Attention! Don't forget to normalize the vectors
			// Use glm::rotate()
			forward = glm::rotate(glm::mat4(1), angle, right) * glm::vec4(forward, 0);
			up = glm::rotate(glm::mat4(1), angle, right) * glm::vec4(up, 0);



			forward = glm::normalize(forward);
			up = glm::normalize(up);

		}

		void RotateFirstPerson_OY(float angle)
		{
			// TODO
			// Compute the new "forward", "up" and "right" vectors
			// Don't forget to normalize the vectors
			// Use glm::rotate()
			

			forward = glm::rotate(glm::mat4(1), angle, glm::vec3(0, 1, 0)) * glm::vec4(forward, 0);
			right = glm::rotate(glm::mat4(1), angle, glm::vec3(0, 1, 0)) * glm::vec4(right, 0);
			//up = glm::rotate(glm::mat4(1), angle, glm::vec3(0, 1, 0)) * glm::vec4(up, 0);
			
			old_forward.x = 0;
			old_forward.y = 0;
			old_forward.z = -1;
			float dot = forward.x * old_forward.x + forward.z * old_forward.z;
			float det = forward.x * old_forward.z - forward.z * old_forward.x;
			aux = atan2(det, dot);


			old_forward.x = 1;
			old_forward.y = 0;
			old_forward.z = 0;
			 dot = forward.x * old_forward.x + forward.z * old_forward.z;
			det = forward.x * old_forward.z - forward.z * old_forward.x;
			aux2 = atan2(det, dot);
			//aux2 = abs(aux2);

			up = glm::cross(right, forward);


			forward = glm::normalize(forward);
			up = glm::normalize(up);
			right = glm::normalize(right);
			
			//printf("aux %f \n", aux);
			
		}

		void RotateFirstPerson_OZ(float angle)
		{
			// TODO
			// Compute the new Right and Up, Forward stays the same
			// Don't forget to normalize the vectors
			right = glm::rotate(glm::mat4(1), angle, forward) * glm::vec4(right, 0);
			up = glm::rotate(glm::mat4(1), angle, forward) * glm::vec4(up, 0);

			up = glm::normalize(up);
			right = glm::normalize(right);

		}

		void RotateThirdPerson_OX(float angle)
		{
			// TODO
			// Rotate the camera in Third Person mode - OX axis
			// Use distanceToTarget as translation distance
			this->TranslateForward(distanceToTarget);
			this->RotateFirstPerson_OX(angle);
			this->TranslateForward(-distanceToTarget);
		}

		void RotateThirdPerson_OY(float angle)
		{
			// TODO
			// Rotate the camera in Third Person mode - OY axis
			this->TranslateForward(distanceToTarget);
			this->RotateFirstPerson_OY(angle);
			this->TranslateForward(-distanceToTarget);
		}

		void RotateThirdPerson_OZ(float angle)
		{
			// TODO
			// Rotate the camera in Third Person mode - OZ axis
			this->TranslateForward(distanceToTarget);
			this->RotateFirstPerson_OZ(angle);
			this->TranslateForward(-distanceToTarget);
		}

		glm::mat4 GetViewMatrix()
		{
			// Returns the View Matrix
		
			return glm::lookAt(position, position + forward, up);
		}


		glm::mat4 GetViewMatrixAuxiliar()
		{
			// Returns the View Matrix
			glm::vec3 auxiliar_position = glm::vec3(0.000176f ,5.144609f, 0.173929f);
			glm::vec3 aux_forward = glm::vec3(0, 0, -1);;
			glm::vec3 aux_up = glm::vec3(0, 1, 0);;
			return glm::lookAt(auxiliar_position, auxiliar_position + aux_forward, aux_up);
	
		}

		glm::mat4 GetViewMatrixBall(glm::vec3 ballPosition)
		{
			// Returns the View Matrix
			glm::vec3 positionB = ballPosition;
			float distance = 0.2;
			positionB = positionB + glm::normalize(up) *distance ;
	
			printf("%f\n", ballPosition.y);

			return glm::lookAt(ballPosition, ballPosition + forward , up);
		}


		glm::vec3 GetTargetPosition()
		{
			return position + forward * distanceToTarget;
		}

	public:
		float distanceToTarget;
		glm::vec3 position;
		glm::vec3 forward;
		glm::vec3 right;
		glm::vec3 old_forward;
		glm::vec3 up;
		float aux;
		float aux2;
	};
}